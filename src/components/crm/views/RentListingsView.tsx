...
import { Home, Search, Plus, Loader2, Eye, Edit, Trash2, ChevronLeft, ChevronRight, Building2, Download } from "lucide-react";
import { fetchListings } from "@/lib/pixxi";

const formatPrice = (n: number) => "AED " + n.toLocaleString("en-US");

const RentListingsView = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchListings({ listingType: "RENT", page, size: 20, name: searchQuery || undefined, sortType });
      setData(res);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-raleway text-xl font-medium text-foreground">Rent Listings</h1>
          <p className="font-raleway text-sm text-muted-foreground">{total} rental properties</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 px-4 py-2 bg-kaya-olive text-primary-foreground rounded-lg text-xs font-raleway font-medium hover:bg-kaya-olive/90"><Plus size={14} /> Add Rent</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 bg-card border border-border rounded-lg p-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            ...
            placeholder="Search rent listings..." className="w-full pl-9 pr-4 py-2 bg-background border border-border rounded-lg font-raleway text-sm focus:outline-none focus:ring-1 focus:ring-kaya-olive/50" />
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-left px-5 py-3 font-raleway text-xs font-medium text-foreground">Rent / Year</th>
          ...
          {listings.length === 0 && <div className="px-5 py-12 text-center"><Home size={32} className="mx-auto text-muted-foreground mb-2" /><p className="font-raleway text-sm text-muted-foreground">No rent listings found</p></div>}
        </div>
      )}
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50">
          <div className="bg-card border border-border rounded-xl w-full max-w-lg mx-4 p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <h2 className="font-raleway text-lg font-medium text-foreground mb-4">Add Rent Listing</h2>
            <div className="space-y-4">
              {["Property Title", "Location / Area", "Annual Rent (AED)", "Bedrooms", "Bathrooms", "Size (sqft)", "Furnishing Status", "Cheques"].map((label) => (
                <div key={label}>
...
export default RentListingsView;
